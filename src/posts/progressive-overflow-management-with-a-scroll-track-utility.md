...
Vous connaissez probablement le vieux scénario classique de conception réactive, dans lequel vous avez une rangée de 3 ou 4 éléments qui ont fière allure dans les grandes fenêtres, mais dans les petites fenêtres, il s’agit d’une longue liste de contenus qui entraînent des blessures répétitives. C’est une situation assez sous-optimale qui peut être extrêmement frustrante pour les utilisateurs. Un truc que j’utilise parfois pour éviter cela est un utilitaire `.scroll-track`.
...
Tout d’abord, il est impossible d’utiliser cet utilitaire sans  avoir “[Bombtrack](https://song.link/s/6ZU9RJIZ0fNaFuQM57bDIA)“ de Rage Against the Machine coincé dans votre tête le reste de la journée. Mis à part ce ver accrocheur, `.scroll-track` est un tout petit utilitaire CSS très performant. Mon type préféré.
